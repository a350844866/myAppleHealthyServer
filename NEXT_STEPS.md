...
本文件记录 `myAppleHealthy` 项目在 `2026-04-09` 这一轮结束时的真实状态，供下次继续开发时直接接手。
...
这一轮做了全面 code review，修复了多个 bug 和架构问题，并补齐了 workout 同步链路。
...
### 10. 全面 code review 修复（2026-04-09）

本轮对后端 + iOS Bridge 做了完整 review，修复了以下问题：

**后端 bug 修复：**

- `isoformat_z()` 修复 tz-aware 分支（之前两个分支行为一样，tz-aware 没有转 UTC）
- `stddev()` 改为样本标准差（`/ (n-1)` 而非 `/ n`）
- `_dashboard_ai_cache` 从无限增长的 dict 改为 TTLCache，修复内存泄漏
- 替换已弃用的 `@app.on_event(“startup”)` 为 lifespan context manager
- 所有 SQL 中的 `CURDATE()` / `NOW()` 替换为 Python 计算的参数化日期（修复时区不一致）
- 连接池创建加双重检查锁（修复多线程竞态）
- `require_ingest_token()` 在 token 未配置时改为拒绝请求（之前放行所有）
- `import_service` 合并重复的 DB 连接块
- ECG 和 body-metrics 接口加分页
- 心率查询简化冗余 CASE WHEN

**Workout 同步链路补齐（关键修复）：**

- iOS Bridge `HealthKitManager` 新增 `fetchWorkoutSamples()` 方法
- 查询 `HKWorkoutType` 并以 `kind: “workout”` 发送到服务端
- 后端 `ingest_service` 新增 workout 处理：接受 `kind: “workout”`，写入 `workouts` 表
- workout 使用 hash 去重 + `ON DUPLICATE KEY UPDATE`
- iOS `shortTypeName` 新增去除 `HKWorkoutActivityType` 前缀

### 11. 累计型指标口径已改成”优先来源解析”
...
10. 前端更完整的自动化测试
    - 详情弹窗交互
    - 路线面板模式切换
11. `HKDataTypeSleepDurationGoal` 同步（仅1条历史数据，优先级极低）
